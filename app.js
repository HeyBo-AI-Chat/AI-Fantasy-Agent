...
async function addSource() {
  ...
  const user_id = await getUserId();
  const team_id = APP.TEAM_ID || 'demo-team-1';
...
function renderLineup() {
  ...
  if (startersBox) {
    startersBox.innerHTML = Array.from(starters).map(id => 
      `<div class="badge">${id}</div>`
    ).join('') || `<div class="badge">No starters selected</div>`;
  }
  
  if (benchBox) {
    benchBox.innerHTML = Array.from(bench).map(id => 
      `<div class="badge">${id}</div>`
    ).join('') || `<div class="badge">No bench selected</div>`;
  }
}
...
function bindButtons() {
  const addSourceBtn = $id('btnAddSource');
  const computeBtn = $id('btnCompute');

  if (addSourceBtn) {
    addSourceBtn.addEventListener('click', async (ev) => {
      ev.preventDefault();
      await addSource();
    });
  }
